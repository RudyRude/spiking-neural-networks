...
use connectivity::{
    PyHypergraphNetwork, PyGraphNetwork, PyMatrixNetwork,
    PySparseMatrixNetwork, PyPlasticConnectivity,
};
use nir::{PyNIRCompiler, PyNIRExecutionResult};
use data_formats::{PyNDFHypergraph, PyDataFormatExporter, PyFormatConverter};
...
fn shnn(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_class::<PyHypergraphNetwork>()?;
    m.add_class::<PyGraphNetwork>()?;
    m.add_class::<PyMatrixNetwork>()?;
    m.add_class::<PySparseMatrixNetwork>()?;
    m.add_class::<PyPlasticConnectivity>()?;

    // NIR compilation classes
    m.add_class::<PyNIRCompiler>()?;
    m.add_class::<PyNIRExecutionResult>()?;

    // Data format classes
    m.add_class::<PyNDFHypergraph>()?;
    m.add_class::<PyDataFormatExporter>()?;
    m.add_class::<PyFormatConverter>()?;
