...
pub use crate::{
    neuron::{Neuron, NeuronId, NeuronType, LIFNeuron, AdExNeuron, IzhikevichNeuron, DetailedLIFNeuron, DetailedHHNeuron, DetailedIzhikevichNeuron},
    spike::{Spike, SpikeTarget, TimedSpike},
    hypergraph::{Hyperedge, HyperedgeId, HypergraphNetwork},
    plasticity::{PlasticityRule, STDPRule},
    time::{Time, TimeStepExt},
    error::{SHNNError, Result},
    connectivity::{NetworkConnectivity, types::SpikeRoute},
    network::{SpikeNetwork, NetworkBuilder},
};
...
pub mod prelude {
    ...
    // Re-export detailed neuron models with original parameters
    pub use crate::neuron::{
        DetailedLIFNeuron, DetailedHHNeuron, DetailedIzhikevichNeuron,
    };
    ...
    pub use crate::network::{
        ...
        builder::{NetworkFactory, PlasticityConfig, NeuronConfig},
    };
    
    // Re-export common type aliases
    pub use crate::{HypergraphSNN, GraphSNN, MatrixSNN, SparseSNN};
...
pub mod compat {
    //! Backward compatibility aliases and deprecated types
    //!
    //! This module provides type aliases and wrapper types to maintain
    //! backward compatibility with existing APIs while encouraging
    //! migration to the new modular system.
    ...
    /// Legacy type alias for hypergraph-based spiking neural networks
    ///
    /// # Migration Guide
    ///
    /// **Old code:**
    ...
    /// **New code:**
    /// ```ignore
    /// use shnn_core::connectivity::hypergraph::HypergraphNetwork;
    /// use shnn_core::network::{SpikeNetwork, NetworkBuilder};
    /// use shnn_core::neuron::{NeuronPool, LIFNeuron};
    ///
    ...
    ///     .unwrap();
    /// ```
    #[deprecated(since = "0.2.0", note = "Use SpikeNetwork with HypergraphNetwork connectivity instead")]
    pub type LegacyHypergraphNetwork = HypergraphNetwork;
}

// Convenient type aliases for common network configurations
/// Hypergraph-based spiking neural network with LIF neurons
pub type HypergraphSNN = SpikeNetwork<crate::hypergraph::HypergraphNetwork, crate::neuron::LIFNeuron>;

/// Traditional graph-based spiking neural network with LIF neurons
pub type GraphSNN = SpikeNetwork<crate::connectivity::graph::GraphNetwork, crate::neuron::LIFNeuron>;

/// Dense matrix-based spiking neural network with LIF neurons
pub type MatrixSNN = SpikeNetwork<crate::connectivity::matrix::MatrixNetwork, crate::neuron::LIFNeuron>;
