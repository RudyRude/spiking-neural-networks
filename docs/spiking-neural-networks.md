...
Leave room for convergence (about 500 steps with 
d
t
=
0.1
...
c
m
 may need to be set such that spiking occurs frequently
...
Dopamine may need longer decay to ensure that weights change afterwards (need to attune 
τ
c
 as well)
...
, 
i
 is index of presynaptic neuron, 
j
 is index of postsynaptic neuron
Weights may need some degree of asymmetry to work properly
Weights may or may not necessarily need inhibitory connections (
k
...
Astrocyte should respond to total glutamate levels of modulated synapses, glutamate input should only have an effect for 
t
 steps after the coherence threshold is met
 Tripartite synapse
